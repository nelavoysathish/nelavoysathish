# Hi, I'm Sathish Nelavoy 👋

🔧 Senior Site Reliability Engineer | DevOps | Cloud Infrastructure

## 🚀 About Me
- 🖥️ Experienced SRE/DevOps Engineer specializing in 
  cloud infrastructure, release automation, and observability
- ☁️ Hands-on with AWS, OpenShift, and Kubernetes 
  across hybrid environments
- 🔁 Passionate about automating everything — 
  from CI/CD pipelines to infrastructure provisioning
- 📍 Based in the United States

## 🛠️ Tech Stack

**Cloud & Infrastructure**
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws)
![OpenShift](https://img.shields.io/badge/OpenShift-EE0000?style=flat&logo=redhatopenshift)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker)

**CI/CD & Automation**
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat&logo=jenkins)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat&logo=ansible)
![GitLab CI](https://img.shields.io/badge/GitLab_CI-FC6D26?style=flat&logo=gitlab)
![ArgoCD](https://img.shields.io/badge/ArgoCD-EF7B4D?style=flat&logo=argo)

**Monitoring & Observability**
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat&logo=prometheus)
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=flat&logo=grafana)
![Splunk](https://img.shields.io/badge/Splunk-000000?style=flat&logo=splunk)

**Languages & Scripting**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python)
![Shell](https://img.shields.io/badge/Shell_Script-121011?style=flat&logo=gnu-bash)
![YAML](https://img.shields.io/badge/YAML-CB171E?style=flat)

## 📌 Featured Projects

| Repo | Description |
|------|-------------|
| 🔧 ansible-playbooks | Automated config management & patching playbooks |
| ☸️ kubernetes-manifests | K8s deployment YAMLs, Helm charts & examples |
| 🚀 cicd-pipelines | Jenkins & GitHub Actions pipeline templates |
| 📊 monitoring-stack | Prometheus rules & Grafana dashboard configs |
| ☁️ aws-infra-terraform | AWS infrastructure as code using Terraform |
| 🐚 sre-scripts | Shell & Python scripts for SRE day-to-day tasks |

## 📫 Connect With Me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin)](https://linkedin.com/in/your-profile)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail)](mailto:nelavoysathish@gmail.com)
